...
def extract_faces(face_detector, image, resolution):
    ...
    for face in faces:
        ...
        image_list.append(image) #add
    return face_list,image_list  #modify
    ...
    for i, file in enumerate(glob.glob(os.path.join(args.input_dir, "*"))):
        ...
        with tqdm(total=int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) // args.sample_rate) as pbar:
            ...
            while cap.grab():
                ...
                face,image = extract_faces(face_detector, img, args.resolution)  #modify
                face_list += face  #modify
                image_list += image
        ...
        top_faces_image = [image_list[i] for i in sorted_indices[:args.num_best]]#add
        ...
        for j, image in enumerate(top_faces_image): 
            cv2.imwrite(os.path.join(args.output_dir, f"{file.rsplit('/',1)[-1].rsplit('.',1)[0]}_{j}_raw.jpg"), image)